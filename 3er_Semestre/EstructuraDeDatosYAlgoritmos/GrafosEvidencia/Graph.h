...
public:
    ...
    void addEdge(T vertex, T adjacent, string month, int day, string hour, string error);
    void setVertices(int qtyVertices);
    void setQtyEdges();
    ...
    int getAdjacenciesOfVertex(T vertex);
...
//setVertices
//toma un vertice input
//lo pone como vertice en grafo
//o(1)
template<class T>
void Graph<T>::setVertices(int qtyVertices){
    this->qtyVertices = qtyVertices;

    vector<Edge<T>> temp;
    vector<vector<Edge<T>>> tempList(this->qtyVertices, temp);
    adjList = tempList;
}

//addVertex
//hace add a un vertice input
//O(1)
template<class T>
...
//addEdge
//hace un add de un edge
//o(1)
template<class T>
void Graph<T>::addEdge(T vertex, T adjacent, string month, int day, string hour, string error){
...
//getAdjacenciesOfVertex
//toma y regresa lista de adyacencia
template<class T>
int Graph<T>::getAdjacenciesOfVertex(T vertex){
    int pos = findVertex(vertex);
    if(pos != -1){
        return adjList[pos].size();
    }
